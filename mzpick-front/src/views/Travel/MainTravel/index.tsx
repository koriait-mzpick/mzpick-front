...
//     getTravelListRequest(accessToken).then(getTravelResponseDto);
//   }
...
export default function MainTravel() {
  ...
  // function: get Travel Response 함수 //
  const getTravelResponseDto = (responseBody: GetTravelListResponseDto | ResponseDto | null) => {
    const message = 
    ...
    const { travel } = responseBody as GetTravelListResponseDto;
    setOriginalList(travel);
    setViewList(travel); // 화면에 표시할 리스트를 설정
  };
      ...
      <div className='board-middle'>
        {viewList.map((item) => (
          <div key={item.travelNumber} className='board-box'>
            <div className='board-image' onClick={() => onItemClickHandler(`${TRAVEL_DETAIL_PATH}/${item.travelNumber}`)}></div>
            <div className='board-information'>
