...
import { Outlet, useNavigate } from 'react-router-dom';
import { useAuthStore } from 'src/stores';
import { HOME_PATH, SIGN_IN_PATH, SIGN_UP_PATH } from '../../constants';
import './style.css';
  ...
  return (
    <div id='main-layout'>
      <div className='layout-logo'>
        ...
        <div className={`navi-box ${sideBarOpen ? 'active' : ''}`}>
          <div className='signin-signup'>
            {cookies.accessToken ? ( // accessToken이 있다면
              <>
                <div className='mypage-button' onClick={() => onItemClickHandler('/mypage')}>마이페이지</div>
                <div className='slice-line' style={{ cursor: "default" }}>/</div>
